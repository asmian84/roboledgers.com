...
class CategorizationEngine {
    ...
    async initialize(allTransactions) {
        ...
        // 1.5 Load Global Vendor Knowledge Base (Phase 1 AI)
        // Check for globally loaded script (File Protocol Safe)
        if (window.GLOBAL_VENDOR_DB) {
            this.globalVendorDB = window.GLOBAL_VENDOR_DB;
            console.log(`🌍 Global Brain Loaded: ${this.globalVendorDB.length} patterns.`);
        } else {
            // Fallback for dev environment or if script block is missing
            try {
                const response = await fetch('./src/data/global_vendor_db.json');
                if (response.ok) {
                    this.globalVendorDB = await response.json();
                    console.log(`🌍 Global Brain Fetched: ${this.globalVendorDB.length} patterns.`);
                }
            } catch (e) {
                console.warn('Global Brain not found (Local or Fetch)', e);
                this.globalVendorDB = [];
            }
        }

        // 1.6 Load Intuition Keywords (Phase 2 AI)
        if (window.KEYWORD_CLUSTERS) {
            this.keywordClusters = window.KEYWORD_CLUSTERS;
            console.log(`🧠 Intuition Engine Loaded: ${Object.keys(this.keywordClusters).length} clusters.`);
        } else {
            try {
                const response = await fetch('./src/data/keyword_clusters.json');
                if (response.ok) {
                    this.keywordClusters = await response.json();
                    console.log(`🧠 Intuition Fetched: ${Object.keys(this.keywordClusters).length} clusters.`);
                }
            } catch (e) {
                console.warn('Intuition Clusters not found (Local or Fetch)', e);
    ...
    classify(txn) {
        ...
        // 1.5. Tier 1 (Enhanced): Global Brain Lookup
        // Check if ANY item in the global DB matches this description
        if (this.globalVendorDB && this.globalVendorDB.length > 0) {
            // We search for a partial match. e.g. "STARBUCKS #123" matches "starbucks" pattern
            const match = this.globalVendorDB.find(rule =>
                normDesc.includes(rule.pattern.toLowerCase())
            );
            if (match) {
                return { category: match.category, confidence: match.confidence, method: 'global_brain' };
            }
        }

        // 1.6 Tier 2 (Enhanced): Intuition Engine (Keyword Heuristics)
        if (this.keywordClusters) {
            const intuition = this._heuristicScore(normDesc);
            if (intuition) {
                return { category: intuition.category, confidence: intuition.confidence, method: 'intuition' };
            }
        }
    ...
    // --- Intuition Engine 1.0 ---
    // Scans description for keywords in defined clusters.
    // e.g. "Joe's Grill" -> Matches "grill" in "Meals" -> Score boost
    _heuristicScore(description) {
        if (!this.keywordClusters) return null;
        ...
        for (const [category, keywords] of Object.entries(this.keywordClusters)) {
            let score = 0;
            // Check for keyword matches
            keywords.forEach(keyword => {
                if (description.includes(keyword)) {
                    // Exact keyword match gives 0.6 confidence base
                    // "Whole word" match is better but "includes" is safer for "Starbucks#123"
                    score += 0.4;
                }
                // Check tokens for exact word match (higher quality)
        ...
        // Threshold for Intuition (0.5 means at least one strong match)
        // Cap confidence at 0.8 because it's still a guess
        if (bestScore >= 0.4) {
            return { category: bestCategory, confidence: Math.min(0.8, bestScore), method: 'intuition' };
        }
        return null;
    }
