...
async function initAccountsGrid() {
  ...
  // ALWAYS USE DEFAULT_CHART_OF_ACCOUNTS and save to storage
  const accountData = DEFAULT_CHART_OF_ACCOUNTS;
  console.log(`📊 Using ${accountData.length} accounts from DEFAULT_CHART_OF_ACCOUNTS`);
  ...
  const gridOptions = {
    ...
    onCellValueChanged: async (event) => {
      if (window.storage) {
        ...
        const updatedAccount = accounts.find(a => a.accountNumber === event.data.code);
        if (updatedAccount) {
          updatedAccount.name = event.data.name;
          await window.storage.setAccounts(accounts);
          console.log('💾 Account updated:', updatedAccount.accountNumber);
        }
      }
    }
  };
...
function addNewAccount() {
  const code = prompt('Enter Account Number:');
  if (!code) return;
  ...
  if (window.storage) {
    window.storage.getAccounts().then(accounts => {
      accounts.push({ accountNumber: code, name: name, id: Date.now() });
      window.storage.setAccounts(accounts);
      if (accountsGridApi) {
        accountsGridApi.applyTransaction({ add: [{ code, name }] });
      }
    });
  }
}
