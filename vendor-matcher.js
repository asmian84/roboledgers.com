...
window.VendorMatcher = {
    ...
    startContinuousOptimization() {
        if (!window.Settings) {
            // console.log('⏳ Waiting for Settings to load before starting AI...');
            setTimeout(() => this.startContinuousOptimization(), 500);
            return;
        }

        if (this._optimizationInterval) {
            console.log('⚠️ Optimization loop already running. Skipping start.');
            return;
        }

        console.log('🚀 Starting Controlled Optimization Loop (Limit: 10 Runs)...');

        let runCount = 0;
        const MAX_RUNS = 10; // User Limit: Stop after 10 batches

        // Run every 1 second (safer than 200ms)
        this._optimizationInterval = setInterval(async () => {
            if (runCount >= MAX_RUNS) {
                console.log('🛑 Optimization Loop Complete (Max Runs Reached). Stopping.');
            ...
            runCount++;
            // console.log(`⚡ Optimization Cycle ${runCount}/${MAX_RUNS}`);

            if (!this.vendors || this.vendors.length === 0) return;

            // BATCH PROCESSING: Grab 5 items (smaller batch)
            const candidates = this.vendors
                .filter(v => (!v._lastChecked || Date.now() - v._lastChecked > 10000))
                .sort((a, b) => (a._lastChecked || 0) - (b._lastChecked || 0))
                .slice(0, 5);
    ...
    async updateVendor(vendorId, updates, options = {}) {
        ...
        if (vendor) {
            ...
            if (window.VendorGrid && window.VendorGrid.gridApi && !window.VendorGrid.gridApi.destroyCalled) {
                // Safety check: Only update if the row actually exists in the grid (prevents console spam)
                // Safety check: Only update if the row actually exists in the grid (prevents console spam)
                const rowNode = window.VendorGrid.gridApi.getRowNode(vendor.id);
                if (rowNode) {
                    // 🛡️ Fix for AG Grid race condition: "cannot get grid to draw rows when it is in the middle of drawing rows"
                    setTimeout(() => {
                        if (window.VendorGrid.gridApi && !window.VendorGrid.gridApi.destroyCalled) {
    ...
    learnFromTransaction(transaction) {
        ...
        if (vendor) {
            // Update existing vendor using standard method (handles Cloud Sync + AI)
            const updates = {
                matchCount: (vendor.matchCount || 0) + 1,
                lastMatched: new Date().toISOString()
            };

            if (!vendor.patterns.includes(transaction.payee)) {
                updates.patterns = [...vendor.patterns, transaction.payee];
            }

            // Update default account if different
            if (vendor.defaultAccount !== transaction.allocatedAccount) {
                updates.defaultAccount = transaction.allocatedAccount;
                updates.defaultAccountName = transaction.allocatedAccountName;
            }

            this.updateVendor(vendor.id, updates);
        } else {
            // Create new vendor
        ...
        // this.saveVendors(); // Handled by updateVendor/createVendor
        return vendor;
    },
