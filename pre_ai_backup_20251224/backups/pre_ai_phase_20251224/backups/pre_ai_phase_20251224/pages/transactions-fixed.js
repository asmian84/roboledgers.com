...
function getChartOfAccounts() {
  ...
  combined.forEach(acc => {
    ...
    // Allow manual category override if present, else guess
    const category = acc.category ? acc.category.toLowerCase() : '';
    const item = { code, name: label, type: acc.type }; // Keep type for filtering

    // Priority: Explicit Category > Type Keyword Match
    if (category.includes('asset') || type.includes('asset')) grouped['Assets'].push(item);
    else if (category.includes('liabil') || type.includes('liab')) grouped['Liabilities'].push(item);
    else if (category.includes('equity') || type.includes('equity')) grouped['Equity'].push(item);
    else if (category.includes('revenue') || category.includes('income') || type.includes('rev') || type.includes('income')) grouped['Revenue'].push(item);
    else if (category.includes('expense') || type.includes('exp')) grouped['Expenses'].push(item);
    else {
...
window.renderTransactions = function () {
  // FORCE REFRESH: Ensure we have the latest data from storage
  // This fixes the issue where imported data wasn't showing until a reload
  if (window.storage) {
    // Sync only if we suspect stale state (or always for safety on route change)
    const fresh = JSON.parse(localStorage.getItem('ab3_transactions') || '[]');
    if (fresh.length !== (window.transactionData || []).length) {
      window.transactionData = fresh;
    }
  } else {
    // Fallback
  ...
  // Totals
  const totalIn = filteredData.reduce((acc, t) => {
    const val = parseFloat(t.credit) || (t.type === 'credit' ? parseFloat(t.amount) : 0) || 0;
    return acc + val;
  }, 0);
  const totalOut = filteredData.reduce((acc, t) => {
    ...
    <div class="transaction-page">
      ...
      <header class="dashboard-header-modern">
        ...
        <div class="header-actions">
          <!-- Import moved to Data Import page -->
        </div>
        <div class="header-stats">
          ...
          <div class="stat-unit">
            ...
            <div id="stat-total-in" class="val green">
              +${fmt(totalIn).replace('$', '')}
            </div>
          </div>
          ...
          <div class="stat-unit">
            ...
            <div id="stat-total-out" class="val red">
          ...
          <div class="stat-unit">
            ...
            <div id="stat-ending-bal" class="val blue">
              ${fmt(ending)}
            </div>
          </div>
        </div>
  ...
  return window.transactionData.map((t, i) => {
    ...
    if (key === 'debit' || key === 'credit') {
      const getVal = (obj, field) => {
        let v = parseFloat(obj[field]);
        if ((isNaN(v) || v === 0) && obj.type === field) {
          v = parseFloat(obj.amount);
        }
        return v || 0;
      };
      valA = getVal(a, key);
...
function reRenderTable() {
  ...
  // UPDATED: Refresh Stats on re-render
  const totalIn = filteredData.reduce((acc, t) => {
    const val = parseFloat(t.credit) || (t.type === 'credit' ? parseFloat(t.amount) : 0) || 0;
    return acc + val;
  }, 0);
  ...
  if (elIn) elIn.innerText = '+' + fmt(totalIn).replace('$', '');
  if (elOut) elOut.innerText = '-' + fmt(totalOut).replace('$', '');
...
window.updateSidebarBadge = function () {
  ...
  const unallocated = data.filter(t => !t.accountDescription || t.accountDescription === 'Uncategorized').length;

  // Find Badge in Sidebar (OUTSIDE of APP div)
  const badge = document.getElementById('sidebar-badge-txn');
  if (badge) {
    badge.innerText = unallocated;
    badge.style.display = unallocated > 0 ? 'inline-block' : 'none';

    // Optional: Pulse animation if > 0
    if (unallocated > 0) {
...
function renderTableRows(data) {
  ...
  return data.map((txn, index) => {
    ...
    // Support both Legacy { debit, credit } and New { amount, type } formats
    let rawDebit = parseFloat(txn.debit || 0);
    let rawCredit = parseFloat(txn.credit || 0);

    // If legacy fields are empty, check for new format
    if (rawDebit === 0 && rawCredit === 0 && txn.amount) {
      if (txn.type === 'debit') rawDebit = parseFloat(txn.amount);
      if (txn.type === 'credit') rawCredit = parseFloat(txn.amount);
    }
    ...
    // Grouped Options (HIERARCHY LAYOUT)
    const coa = getChartOfAccounts();
    // USER REQUEST: Strict Order
    const categories = ['Assets', 'Liabilities', 'Equity', 'Revenue', 'Expenses', 'Other'];

    dropdownHtml += `<div class="smart-menu-body" style="max-height: 250px; overflow-y: auto; overflow-x: hidden;">`;

    categories.forEach(cat => {
      const options = coa[cat] || [];
      if (options.length > 0) {

        // Category Header (Collapsible)
        const catId = `cat-${realIndex}-${cat}`;
        dropdownHtml += `
          <div class="smart-menu-header" onclick="event.stopPropagation(); window.toggleSmartCategory('${catId}')" style="
            font-weight: 700; color: #64748b; font-size: 0.75rem; 
            text-transform: uppercase; letter-spacing: 0.05em; 
            padding: 8px 12px; border-bottom: 1px solid #f1f5f9; background: #f8fafc;
            position: sticky; top: 0; z-index: 1; cursor: pointer; display: flex; justify-content: space-between; align-items: center;">
            <span>${cat}</span>
            <span id="icon-${catId}" style="font-size: 0.8rem; transition: transform 0.2s;">▶</span>
          </div>`;

        // Options Container (Hidden by default)
        dropdownHtml += `<div id="${catId}" class="smart-category-group" style="display: none;">`;

        // Options
        // Sort by CODE (Numeric) -> Fallback to Name
        options.sort((a, b) => {
          const cA = parseInt(a.code || 0);
          const cB = parseInt(b.code || 0);
          if (cA === 0 && cB === 0) return a.name.localeCompare(b.name);
          if (cA === 0) return 1;
          if (cB === 0) return -1;
          return cA - cB;
        });
        ...
        options.forEach(item => {
          ...
          dropdownHtml += `
            <div class="smart-menu-item" onclick="selectAccount(${realIndex}, '${safeAcc}')" style="padding-left: 24px; border-left: 3px solid transparent;" data-parent="${catId}">
              ${accName}
            </div>`;
        });

        dropdownHtml += `</div>`; // Close group container
      }
    });
      ...
      <tr>
        ...
        <td class="w-account">
          <div class="smart-dropdown-wrapper">
            <div class="smart-pill"
              style="padding: 4px 12px; cursor: pointer; border: 1px solid #cbd5e1; background: white; display: flex; align-items: center; justify-content: space-between;"
              onclick="window.toggleDropdown(event, ${realIndex})"
              tabindex="0"
              onkeydown="window.handlePillKey(event, ${realIndex})">
              
              <span class="smart-pill-text" style="font-size: 0.9rem; font-weight: 500; color: #334155; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 180px;">
                ${accountLabel}
              </span>
        ...
        <td class="w-actions">
          <div style="display: flex; gap: 4px; justify-content: center; height: 100%; align-items: center;">
            <button onclick="window.actionSwap(${realIndex})" title="Swap Debit/Credit" 
                    style="border: none; background: none; cursor: pointer; opacity: 0.6; padding: 4px; font-size: 1.1rem; transition: opacity 0.2s;"
                    onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.6">
                ⇄
            </button>
            ${isConfirmingDelete
        ? `<span class="delete-confirm" onclick="confirmDelete(${realIndex})" style="font-size: 0.75rem; white-space: nowrap; cursor: pointer;">Confirm? <b style="color:#ef4444;">Yes</b> / <b onclick="cancelDelete(${realIndex}, event)" style="color:#64748b;">No</b></span>`
        : `<button onclick="requestDelete(${realIndex})" title="Delete"
                         style="border: none; background: none; cursor: pointer; opacity: 0.6; padding: 4px; font-size: 1.1rem; color: #ef4444; transition: opacity 0.2s;"
                         onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.6">
                   ✕
                 </button>`
      }
...
// --- SWAP LOGIC (MATCHING IMPORT ENGINE) ---
window.actionSwap = function (index) {
  // Use the master data source
  const data = window.transactionData || window.transactionState.data;
  if (!data) {
  ...
  if (!txn) {
    ...
    if (window.showToast) window.showToast('Error: Transaction not found', 'error');
    return;
  }

  // Smart Swap Logic - Explicit Resolution
  let valDebit = parseFloat(txn.debit || 0);
  let valCredit = parseFloat(txn.credit || 0);

  // Normalize if legacy fields are empty (Handles PDF Import format)
  if (valDebit === 0 && valCredit === 0 && txn.amount) {
    const t = (txn.type || '').toLowerCase();
    if (t === 'debit') valDebit = parseFloat(txn.amount);
    else if (t === 'credit') valCredit = parseFloat(txn.amount);
  }

  // Perform Swap
  if (valCredit > 0) {
    // Was Credit -> Swap to Debit
    txn.debit = valCredit;
    txn.credit = 0;
    txn.type = 'debit';
    txn.amount = valCredit;
  } else if (valDebit > 0) {
    // Was Debit -> Swap to Credit
    txn.credit = valDebit;
    txn.debit = 0;
    txn.type = 'credit';
    txn.amount = valDebit;
  } else {
    // Both zero? Just flip type tag to signal intent
    txn.type = (txn.type === 'debit') ? 'credit' : 'debit';
  }

  console.log('🔄 Swapped info:', txn);

  // Feedback
  if (window.showToast) window.showToast('Swapped Debit ⇄ Credit', 'success');
...
// ============================================
// DROPDOWN UTILITIES (RESTORED)
// ============================================

window.openDropdown = function (id) {
  // 1. Resolve Element (Direct ID or Prefixed)
  let dropdown = document.getElementById(id);
  // If not found, try the 'dropdown-' prefix convention used by Grid
  if (!dropdown) {
  ...
  // 2. Identify Type (Smart Grid Menu vs Generic Dropdown)
  const isSmart = dropdown.classList.contains('smart-menu');

  if (isSmart) {
    // --- SMART MENU LOGIC (Inline Styles) ---
    const isVisible = dropdown.style.display === 'block';

    // Close Others
    document.querySelectorAll('.smart-menu').forEach(d => {
      if (d !== dropdown) d.style.display = 'none';
    });
    // Also close generic ones to be safe
    ...
    // Also close smart ones
    document.querySelectorAll('.smart-menu').forEach(d => d.style.display = 'none');
...
// --------------------------------------------
// KEYBOARD NAVIGATION (JUMP-TO)
// --------------------------------------------
window.searchBuffer = '';
window.searchBufferTimer = null;

// Collapsible Logic
window.toggleSmartCategory = function (id) {
...
window.handlePillKey = function (e, index) {
  // 1. Handle Special Keys
  if (e.key === 'Enter') {
    window.toggleDropdown(e, index);
    return;
  }
  if (e.key === 'Escape') {
  ...
  // 2. Filter for Character Keys only (A-Z, 0-9)
  if (e.key.length !== 1) return;

  // 3. Buffer Logic (allow typing "ME" to match Meals)
  // Check if buffer exists
  if (typeof window.searchBuffer === 'undefined') window.searchBuffer = '';

  clearTimeout(window.searchBufferTimer);
  window.searchBuffer += e.key.toLowerCase();

  window.searchBufferTimer = setTimeout(() => {
    window.searchBuffer = '';
  }, 1000); // Reset after 1 second of inactivity

  const term = window.searchBuffer;
  const dropdown = document.getElementById(`dropdown-${index}`);
  ...
  // Re-fetch in case toggle failure or async
  const openDropdown = document.getElementById(`dropdown-${index}`);
  if (!openDropdown) return;

  // 4. Find Match
  const items = openDropdown.querySelectorAll('.smart-menu-item'); // Clickable items
  ...
  items.forEach(i => {
    ...
    i.classList.remove('active-jump-item');
  });
  ...
  // 5. Scroll & Highlight & EXPAND
  if (found) {
    found.style.backgroundColor = '#dbeafe';
    found.classList.add('active-jump-item');

    // Auto-Expand Parent
    const parentId = found.getAttribute('data-parent');
    if (parentId) {
...
window.selectDropdownOption = function (txnId, field, value) {
  // 1. Update Data
  const txn = window.transactionData.find(t => t.id === txnId);
  if (txn) {
    txn[field] = value;
    if (field === 'category') txn.reconciled = true; // Auto-verify on modify
    window.saveTransactions(); // Use the global function
  }
  ...
  // 3. Toast
  if (window.showToast) window.showToast(`Updated ${field} to ${value}`);
};

// Global click to close dropdowns
document.addEventListener('click', (e) => {
  if (!e.target.closest('.custom-select-container') && !e.target.closest('.dropdown-trigger') && !e.target.closest('.dropdown-menu')) {
    document.querySelectorAll('.dropdown-menu').forEach(d => d.classList.add('hidden'));
  }
});
