...
class VendorEngine {
    ...
    trainBayes() {
        ...
        this.vendors.forEach(v => {
            ...
            // Count category frequency (Weighted)
            const weight = v.frequency || 1;
            this.categoryCounts[category] = (this.categoryCounts[category] || 0) + weight;
            ...
            tokens.forEach(token => {
                ...
                // Weighted learning
                this.bayesModel[token][category] = (this.bayesModel[token][category] || 0) + weight;
            });
        });
    }
     ...
     */
    async learn(rawDescription, accountId, meta = {}) {
        ...
        if (vendor) {
            ...
            let updates = {};
            if (vendor.defaultAccountId !== accountId) updates.defaultAccountId = accountId;

            // Merge metadata if provided (e.g. accountNumber)
            if (meta && Object.keys(meta).length > 0) {
                updates = { ...updates, ...meta };
            }

            if (Object.keys(updates).length > 0) {
                // If frequency is provided in meta, merge it
                if (meta.frequency) {
                    updates.frequency = (vendor.frequency || 1) + meta.frequency;
                }

                await window.storage.updateVendor(vendor.id, updates);
                console.log(`🧠 VendorEngine: Updated rule for '${vendor.name}' -> ${accountId}`, updates);
            }
        } else {
            // Create new vendor
            ...
            const newVendor = {
                ...
                category: '',
                frequency: meta.frequency || 1, // Store Initial Frequency
                ...meta // Spread metadata like { accountNumber: '123' }
            };
            await window.storage.createVendor(newVendor);
            console.log(`🧠 VendorEngine: Learned new vendor '${cleanName}' -> ${accountId} (Freq: ${newVendor.frequency})`);
        }
