...
function initializeDataImport() {
    // RESET STATE to prevents stale grid references on navigation
    window.previewGrid = null;
...
async function parseFilesToChunks(files) {
    ...
    for (const file of files) {
        ...
        if (fileName.endsWith('.pdf')) {
            ...
            // 🛑 Entry-Level Deduplication (The Data Junkie Filter - Level 2)
            const existingTxns = JSON.parse(localStorage.getItem('ab3_transactions') || '[]');
            const existingHashes = new Set(existingTxns.map(t =>
                `${t.date || t.Date}-${parseFloat(t.amount || t.Amount).toFixed(2)}-${(t.description || t.Description || '').trim().toLowerCase()}`
            ));

            let duplicateCount = 0;
            const uniqueMapped = mapped.filter(t => {
                const hash = `${t.Date}-${parseFloat(t.Amount).toFixed(2)}-${t.Description.trim().toLowerCase()}`;
            ...
            if (duplicateCount > 0) {
                console.warn(`🛑 Skipped ${duplicateCount} duplicate transactions from ${fileName}.`);
                window.showToast(`Skipped ${duplicateCount} duplicates`, 'warning');
            }

            parsedData = uniqueMapped;
        ...
        if (parsedData && parsedData.length > 0) {
            ...
            window.showToast(`File ${file.name} contained only duplicates.`, 'warning');
        }
    }
...
window.loadToTransactions = async function () {
    ...
    try {
        const selection = await window.selectTargetAccount();
        targetAccount = selection.account;
        shouldClear = selection.clearData;
    } catch (e) {
    ...
    if (shouldClear) {
        ...
        if (window.allTransactionData) window.allTransactionData = [];
    }
    ...
    if (!targetAccount || !targetAccount.id) {
        alert('An account must be selected to save transactions.');
        return;
    }

    window.showToast('Importing transactions...', 'info');

    // Process sequentially to ensure correct order
    for (const txn of data) {
        ...
        const transactionObj = {
            ...
            originalDescription: txn.OriginalDescription || txn.Description, // Save raw
            amount: amount,
            type: type,
            category: 'Uncategorized',
            accountId: targetAccount.id, // Critical: Tag with Source Account
            reconciled: false
        };
    ...
    // VERIFICATION: Log to console for user to see
    console.group('✅ Import Verification');
    console.log('Use this to check if original text is preserved:');
    const recent = data.slice(0, 5).map(t => ({
        Clean: t.Description,
        Original: t.OriginalDescription || t.Description
    }));
    console.table(recent);
    console.groupEnd();

    // Trigger Auto-Categorize & Navigate
    setTimeout(() => {
        // Clear session
        localStorage.removeItem('ab_import_session');
...
// Account Selection Modal (Standard System UI)
window.selectTargetAccount = function () {
    return new Promise((resolve, reject) => {
        let accounts = window.accountManager ? window.accountManager.getAllAccounts() : [];
        if (accounts.length === 0) {
            // Should be seeded now, but fallback just in case
            alert('Initializing Default Accounts...');
            window.accountManager.initialize(); // Trigger default seed
            accounts = window.accountManager.getAllAccounts();
        }

        const modalDiv = document.createElement('div');
        modalDiv.className = 'custom-modal-backdrop'; // Use System Backdrop
        modalDiv.style.display = 'flex'; // Ensure visible

        modalDiv.innerHTML = `
            <div class="custom-modal-container" style="max-width: 450px;">
                <div class="custom-modal-header">
                    <h3 id="modal-title">Select Target Account</h3>
                    <button class="custom-modal-close" id="close-acc-select">&times;</button>
                </div>
                <div class="custom-modal-body">
                    <p style="margin-bottom: 16px; color: #64748b; font-size: 0.95rem;">
                        Where should these transactions be imported?
                    </p>
                    ...
                    <div style="position: relative;">
                        ...
                        <select id="target-account-select" style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.95rem; color: #1e293b; background-color: #f8fafc; cursor: pointer; height: 44px;">
                            ${accounts.map(acc => `<option value="${acc.id}">
                                ${acc.type === 'bank' ? '🏦' : '💳'} &nbsp; ${acc.accountName || acc.name}
                            </option>`).join('')}
                            <option value="create_new" style="font-weight: 600; color: #2563eb;">
                                ✚ &nbsp; Create New Account
                            </option>
                        </select>
                        <div style="pointer-events: none; position: absolute; right: 12px; top: 38px; color: #64748b;">▼</div>
                    </div>
                </div>
                <div class="custom-modal-footer" style="display:flex; align-items:center; justify-content:space-between;">
                     <div style="display:flex; align-items:center; gap:8px;">
                        <input type="checkbox" id="clear-data-checkbox" style="width:16px; height:16px; cursor:pointer;">
                        <label for="clear-data-checkbox" style="font-size:0.85rem; color:#ef4444; font-weight:600; cursor:pointer;">Clear existing data?</label>
                     </div>
                     <div style="display:flex; gap:8px;">
                        <button class="btn-modal btn-modal-cancel" id="cancel-account-select">Cancel</button>
                        <button class="btn-modal btn-modal-primary" id="confirm-account-select">
                            Continue Import
                        </button>
                     </div>
                </div>
        ...
        modalDiv.querySelector('#close-acc-select').onclick = close;
        modalDiv.querySelector('#cancel-account-select').onclick = close;

        const selectEl = modalDiv.querySelector('#target-account-select');

        // Watch for "Create New" selection
        selectEl.addEventListener('change', () => {
            if (selectEl.value === 'create_new') {
                modalDiv.style.display = 'none'; // Hide temporarily
                // Use the existing Account Switcher modal logic
                if (window.accountSwitcher) {
                    window.accountSwitcher.showAddAccountModal();

                    // Poll for new account creation
                    const checkInterval = setInterval(() => {
                        const current = window.accountManager.getCurrentAccount();
                        // If a new account became active (creation success)
                        if (current && !accounts.find(a => a.id === current.id)) {
                            clearInterval(checkInterval);
                            modalDiv.remove(); // Remove old modal
                            resolve({ account: current, clearData: false }); // Resolve with new account, assume no clear data
                        }
                        // If user cancelled creation (e.g. clicked X on add modal), we might detect state or just rely on them reopening. 
                        // Ideally we'd pass a callback to showAddAccountModal, but it doesn't support it yet.
                    }, 500);
                } else {
                    alert('Account creator not loaded.');
                    modalDiv.style.display = 'flex'; // Show back
                    selectEl.value = accounts[0].id; // Reset
                }
            }
        });
        ...
        modalDiv.querySelector('#confirm-account-select').onclick = () => {
            ...
            const clearData = modalDiv.querySelector('#clear-data-checkbox').checked;

            if (accountId === 'create_new') {
                // Should have been handled by change event, but if they clicked fast:
