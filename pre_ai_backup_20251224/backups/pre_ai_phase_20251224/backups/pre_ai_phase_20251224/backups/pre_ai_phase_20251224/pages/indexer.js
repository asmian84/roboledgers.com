...
window.renderIndexerPage = function () {
    ...
    <style>
        /* ... Existing CSS ... */
        .indexer-container { padding: 24px; max-width: 100%; height: calc(100vh - 80px); display: flex; flex-direction: column; gap: 20px; font-family: 'JetBrains Mono', 'Courier New', monospace; box-sizing: border-box; }
        ...
        .indexer-main-layout { display: flex; gap: 24px; flex: 1; min-height: 0; overflow: hidden; }
        ...
        .control-panel { width: 320px; gap: 20px; flex-shrink: 0; overflow-y: auto; }
        .indexer-right-panel { flex: 1; display: flex; flex-direction: column; gap: 16px; min-width: 0; }
        .grid-panel { flex: 2; padding: 0; overflow: hidden; display: flex; flex-direction: column; }
        ...
        .terminal-panel { flex: 1; background: #0f172a; border: 1px solid #334155; color: #e2e8f0; padding: 0; overflow: hidden; min-height: 200px; }
...
async function processPDF(file, filename) {
    ...
    try {
        ...
        // It handles text extraction, bank identification, and transaction parsing
        ...
        if (e.message.includes('DUPLICATE')) {
            log(`Skipped ${filename}: Duplicate file.`, 'warn');
        } else {
