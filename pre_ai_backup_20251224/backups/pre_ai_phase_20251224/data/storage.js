...
class StorageService {
    ...
    async getVendors() {
        // HYBRID: Try BrainStorage (IndexedDB) first
        let vendors = [];
        if (window.BrainStorage) {
            try {
                vendors = await window.BrainStorage.getAllVendors();
            } catch (e) {
                console.warn('BrainStorage unavailable for vendors, falling back to local.', e);
    ...
    async createVendor(data) {
        ...
        if (data.frequency) vendor.frequency = data.frequency;

        // Persist to BrainStorage if available
        if (window.BrainStorage) {
            await window.BrainStorage.saveVendor(vendor);
        } else {
    ...
    async updateVendor(id, updates) {
        // BrainStorage Path
        if (window.BrainStorage) {
            const allVendors = await window.BrainStorage.getAllVendors();
            const index = allVendors.findIndex(v => v.id === id);
            ...
            await window.BrainStorage.saveVendor(updated);
            console.log('💾 Vendor updated (DB):', id);
            return updated;
        }
    ...
    async deleteVendor(id) {
        if (window.BrainStorage) {
            await window.BrainStorage.deleteVendor(id);
            return true;
        }
