...
class AccountSwitcher {
  ...
  showAddAccountModal() {
                ...
                <form id="create-account-form" onsubmit="return false;">
                    ...
                    <div class="form-row" style="display: flex; gap: 16px; margin-top: 16px;">
                        <div class="form-group" style="flex: 1;">
                            <label>GL Account Code (Linked)</label>
    ...
    /* Auto-Suggest GL Logic */
    const updateGLSuggestion = () => {
      const type = modal.querySelector('input[name="accountType"]:checked').value;
      const numInput = document.getElementById('new-account-number');
      // Simple heuristic: Bank = 10xx, Credit = 21xx
      // In a real app, we'd query existing accounts to find next max.
      // For now, let's use a random safe start to avoid collision or just standard defaults
      if (type === 'bank') numInput.value = '10' + Math.floor(10 + Math.random() * 89);
      else numInput.value = '21' + Math.floor(10 + Math.random() * 89);
    };

    modal.querySelectorAll('input[name="accountType"]').forEach(r => r.addEventListener('change', updateGLSuggestion));

    // Save Action
    document.getElementById('save-account-btn').onclick = async () => {
      const name = document.getElementById('new-account-number').dataset.name_value || document.getElementById('new-account-name').value.trim();
      // Fix: get value properly
      const nameVal = document.getElementById('new-account-name').value.trim();
      ...
      if (!/^\d{4}$/.test(number)) {
        alert('Please enter a valid 4-digit GL Code.');
        return;
      }

      try {
        const newAccount = await this.accountManager.createAccount({
          accountName: nameVal,
          type: type,
        ...
        if (window.toast) {
          window.toast.success(`Created & Linked Account: ${nameVal} (${number})`);
        }
        close();
      } catch (error) {
        alert(error.message);
