        ...
        if (vendorSummaryBtn) {
            vendorSummaryBtn.addEventListener('click', () => {
                ...
                if (vendorSummaryModal) {
                    ...
                    // 🔍 REFINED LOGIC (v125): Load Aggregated Stats (Multi-Account Aware)
                    if (window.VendorManager && VendorManager.loadVendorStats) {
                        const stats = VendorManager.loadVendorStats();
                        console.log(`📊 Loaded ${stats ? stats.length : 0} vendors for filtering.`);
                        // Load into Grid
                        if (window.VendorSummaryGrid) {
                            VendorSummaryGrid.loadVendors(stats || []);
                        }
                    } else {
                        console.warn('VendorManager.loadVendorStats not found.');
                    }
                }
            });
        }
...
if (settingsAccountsBtn) {
    settingsAccountsBtn.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
